    ...
    public void drawInterface(int tier) {
        ...
        for (Achievement achievement : Achievement.ACHIEVEMENTS) {
            if (achievement.getTier().ordinal() == tier) {
                ...
                player.getPA().sendFrame126(achievement.name().substring(0, 1).toUpperCase() + achievement.name().substring(1).toLowerCase().replaceAll("_", " "), count);
                count++;
            }
        }
    ...
    public void viewAchievement(int achievement, int tier) {
        ...
        for (Achievement ach : Achievement.ACHIEVEMENTS) {
            if (ach.getTier().ordinal() == tier) {
                ...
                if (count == toView) {
                    int descLines;
                    player.getPA().sendFrame126(ach.name().substring(0, 1).toUpperCase() + ach.name().substring(1).toLowerCase().replaceAll("_", " "), 36501);
