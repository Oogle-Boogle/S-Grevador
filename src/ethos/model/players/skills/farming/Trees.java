...
public class Trees {
	...
	public void handleTree(Player c, int treeId, int seed, int x, int y) {
		...
		double osrsExperience = Tree.getPlantingXp() + Tree.getPlantingXp() / 20 * pieces;
		double regExperience = Tree.getPlantingXp() * (Config.FARMING_EXPERIENCE + (hasMagicSecateurs() ? 1 : 0)) + Tree.getPlantingXp() * Config.FARMING_EXPERIENCE / 20 * pieces;
		switch(treeId) {
			case FarmingConstants.OAK_TREE:
				...
				c.getPA().addSkillXP((int) (c.getMode().getType().equals(ModeType.OSRS) ? osrsExperience : regExperience), 19, true);
			...
			case FarmingConstants.OAK_TREE:
				if(c.getLogTimer() <= 0) {
					if(c.getLogAmount() > 0) {
						...
						c.getPA().addSkillXP(HarvestXP(treeId) * Config.FARMING_EXPERIENCE, 19, true);
