...
public class Prayer {
	...
	public void alter(final int amount) {
		...
		player.getPA().addSkillXP((int) (bone.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? 1 * 3 : (Config.PRAYER_EXPERIENCE * player.prestige()) * 3)), Skill.PRAYER.getId(), true);
		player.getItems().deleteItem2(bone.getItemId(), 1);
