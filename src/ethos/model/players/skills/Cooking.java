...
public class Cooking extends SkillHandler {
	...
	private static void cookFish(Player c, int itemID, int xpRecieved, int levelRequired, int burntFish, int cookedFish, int object) {
		...
		c.playerSkillProp[7][1] = (int) (xpRecieved * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.COOKING_EXPERIENCE * c.prestige()));
	...
	private static void cookTheFish(final Player c) {
		...
		if (c.playerSkillProp[7][5] > 0) {
			c.startAnimation(c.playerSkillProp[7][5] == 5249 || c.playerSkillProp[7][5] == 3769 || c.playerSkillProp[7][5] == 26185 ? 897 : 896);
		}
		c.getSkilling().stop();
