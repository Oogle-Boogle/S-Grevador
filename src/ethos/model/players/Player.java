...
public class Player extends Entity {
	...
	public void highscores() {
		getPA().sendFrame126("Grevador - Top PKers Online", 6399); // Title
		for (int i = 0; i < 10; i++) {
			if (ranks[i] > 0) {
	...
	public void process() {
		...
		if (this.playTime % 3600 == 0 && !isIdle && !trading){
			if (getHourlyBoxToggle()) {
				...
				sendMessage("[ @red@Reward @bla@] <img=1>@blu@Thank you for continuing to play Grevador, here is an @red@Hourly Reward Box@bla@!");
			} else {
